...
現在は **第12回：クラウド同期の土台（DB・RLS・手動での保存／取得）** まで対応。入力した内容はこのブラウザの localStorage に保存される（クラウドへ送るのは、ログイン中にデータ管理でボタンを押したときだけ）。
...
| `docs/cloud-sync-design.md` | 第10回のクラウド同期 設計書 |
| `docs/cloud-sync-round12.md` | 第12回で作ったDB・RLS・同期の説明 |
| `supabase/` | Supabase のテーブル・RLS の SQL（秘密情報は含まない） |
...
## クラウド同期（第11回：ログイン／第12回：手動での保存・取得）

- データの正本は**いつもこの端末（localStorage）**。クラウドは同期先で、**自動では一切やりとりしない**。
- 環境変数を設定したビルドでだけ、データ管理に「アカウント・クラウド同期」欄が有効になり、メールのリンク（Magic Link）でログイン・ログアウトできる。
- ログイン中は、ボタンを押したときだけ次ができる。
  - **クラウドの状態を確認**（件数だけを取得）
  - **この端末のデータをクラウドへ保存**（プレビュー → 必要なら確認 → 実行）
  - **クラウドのデータをこの端末へ取得**（プレビュー → バックアップ保存 → 確認 → 実行。直後なら「1つ前の状態に戻す」で戻せる）
- 上書きで消える側がある場合は実行できない（クラウドが空のときの取得、端末が空のときの保存）。詳しくは `docs/cloud-sync-round12.md`。
- Supabase 側のテーブルと RLS は `supabase/migrations/` の SQL。適用手順は `supabase/README.md`。
